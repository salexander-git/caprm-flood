...
#   .\verify_submission.ps1 -Pdf "$HOME\Downloads\sa5836_Report_Final.pdf"
#
...
if (-not (Test-Path $Pdf)) {
    Write-Host "FAIL  file not found: $Pdf" -ForegroundColor Red
    exit 1
}
...
$tmp = Join-Path $env:TEMP "verify_submission_check.py"
...
if ($code -eq 0) {
    ...
    Write-Host "`nOne or more checks failed - review above before submitting." -ForegroundColor Yellow
}
exit $code
